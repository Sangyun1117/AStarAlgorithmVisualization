...
void Engine::ScreenSettings()
{
	for (int i = 0; i < 2; ++i) {
		...
		// 0. 콘솔 폰트 크기 설정 (버퍼 크기 설정 전에 먼저)
		CONSOLE_FONT_INFOEX fontInfo = {};
		fontInfo.cbSize = sizeof(CONSOLE_FONT_INFOEX);
		if (GetCurrentConsoleFontEx(hOut, FALSE, &fontInfo)) {
			// 폰트 크기 설정
			fontInfo.dwFontSize.X = 0;  // 0으로 설정하면 폰트 비율에 맞춰 자동 계산
			fontInfo.dwFontSize.Y = 32;  // 폰트 높이 (원하는 크기로 조정)
			fontInfo.FontFamily = FF_DONTCARE;
			fontInfo.FontWeight = FW_NORMAL;
			wcscpy_s(fontInfo.FaceName, L"Consolas");
			if (!SetCurrentConsoleFontEx(hOut, FALSE, &fontInfo)) {
				std::cerr << "SetCurrentConsoleFontEx 실패 buffer" << i << "\n";
			}
		}
		else {
			std::cerr << "GetCurrentConsoleFontEx 실패 buffer" << i << "\n";
		}
